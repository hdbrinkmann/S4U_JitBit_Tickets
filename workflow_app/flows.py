...
from .steps import (
    ...
    # WebCRM steps
    step_webcrm_download_documents,
    StepResult
)
from .envcheck import check_jitbit_env, check_jira_env, check_webcrm_env, check_llm_env
...
def validate_env_for_flow(flow_name: str) -> tuple:
    ...
    if flow_name == "jitbit":
        ...
        # Check WebCRM environment
        webcrm_results = check_webcrm_env()

        for result in webcrm_results:
            if not result.is_ok:
                errors.append(result.message)
    else:
...
def run_webcrm_flow(params: Dict[str, Any], options: Dict[str, Any] = None,
                    run_id: str = None, run_dir: Path = None) -> FlowResult:
    """
    Run the complete WebCRM workflow.
    ...
    try:
        log_run_start(logger, "WebCRM", params)

        # Step 1: Validate environment
        logger.step_start("Validate Environment")
        update_step_status(run_dir, "Validate Environment", "running")

        env_valid, env_errors = validate_env_for_flow("webcrm")
        if not env_valid:
            error_msg = "Environment validation failed: " + "; ".join(env_errors)
            logger.error(error_msg)
            logger.step_end("Validate Environment", success=False)
            ...
            duration = time.time() - start_time
            log_run_end(logger, "WebCRM", False, duration)
        ...
        # Step 2: Download WebCRM documents
        result = step_webcrm_download_documents(params, run_dir, logger, options)
        step_results.append(result)
        if not result.success:
            duration = time.time() - start_time
            log_run_end(logger, "WebCRM", False, duration)
        ...
        # Success!
        duration = time.time() - start_time
        log_run_end(logger, "WebCRM", True, duration)

        return FlowResult(
            success=True, run_id=run_id, run_dir=run_dir,
            message="WebCRM workflow completed successfully",
            step_results=step_results, duration_seconds=duration
        )

    except Exception as e:
        duration = time.time() - start_time
        error_msg = f"Unexpected error in WebCRM workflow: {e}"
        logger.error(error_msg)
        log_run_end(logger, "WebCRM", False, duration)
